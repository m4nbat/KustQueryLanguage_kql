...
| ---  | --- | --- |
| T1003.001 | OS Credential Dumping: LSASS Memory | [OS Credential Dumping: LSASS Memory](https://attack.mitre.org/techniques/T1003/001/) |
| T1548.002 | Abuse Elevation Control Mechanism: Bypass User Account Control | [Abuse Elevation Control Mechanism: Bypass UAC](https://attack.mitre.org/techniques/T1548/002/) |
...
| T1550.002 | Use Alternate Authentication Material: Pass the Hash | [Use Alternate Authentication Material: Pass the Hash](https://attack.mitre.org/techniques/T1550/002/) |

#### Description
Detection queries for BumbleBee malware activity as documented in the DFIR Report from November 2022. BumbleBee is a loader used as an initial access mechanism that delivers payloads including Meterpreter and Cobalt Strike. These queries cover post-exploitation techniques observed including credential dumping via procdump/LSASS, UAC bypass methods, PowerShell exploitation frameworks, privilege escalation via getsystem, and discovery using ADFind and nltest.

#### Risk
BumbleBee infections lead to hands-on-keyboard intrusions using professional offensive tools (Cobalt Strike, Meterpreter). Detection of the documented TTPs is critical to preventing ransomware deployment and full domain compromise.
